...
class PortailDouble {
  ...
  pollcb() {
    ...
    this.log.debug("Contact on pin P%d has set to %s, (%s after 20ms)", this.Contact_GPIO, value, value2);
    if (value != value2) {
    ...
    if (old_state != new_state) {
      this.debug.log("Update contact sensor state from %s to: %s", old_state, new_state);
    }
