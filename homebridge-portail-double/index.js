...
function PortailDouble(log, config) {
  ...
  this.GrandeOuverture_temporaryOnTimeMS =
    config.GrandeOuverture_temporaryOnTimeMS;
  ...
  this.OuverturePieton_temporaryOnTimeMS =
    config.OuverturePieton_temporaryOnTimeMS;
  ...
  this.Contact_gpio.watch(
    function (err, value) {
      ...
      setTimeout(
        this.Contact_processGPIOWatch.bind(this),
        this.Contact_holdoffMS,
        this.Contact_lastCallerIdentifier
      );
    }.bind(this)
  );

  this.GrandeOuverture_onCommandValueIsHigh =
    config.GrandeOuverture_onCommandValue == 0 ? 0 : 1;
  this.GrandeOuverture_gpio = new Gpio(
    config.GrandeOuverture_BCM_GPIO,
    this.GrandeOuverture_onCommandValueIsHigh ? "low" : "high"
  );

  this.OuverturePieton_onCommandValueIsHigh =
    config.OuverturePieton_onCommandValue == 0 ? 0 : 1;
  this.OuverturePieton_gpio = new Gpio(
    config.OuverturePieton_BCM_GPIO,
    this.OuverturePieton_onCommandValueIsHigh ? "low" : "high"
  );
...
PortailDouble.prototype = {
  ...
  setGrandeOuverture_GPIO: function (switchState) {
    this.GrandeOuverture_gpio.writeSync(
      this.GrandeOuverture_onCommandValueIsHigh ^ (switchState ? 0 : 1)
    );
  },
  ...
  setOuverturePieton_GPIO: function (switchState) {
    this.OuverturePieton_gpio.writeSync(
      this.OuverturePieton_onCommandValueIsHigh ^ (switchState ? 0 : 1)
    );
  },
  ...
  setOuverturePieton_SwitchState: function (powerOn, callback) {
    ...
    if (powerOn && this.OuverturePieton_autoCloseS != undefined) {
      ...
      if (this.currentDoorState() == Characteristic.CurrentDoorState.CLOSED) {
        var additionalTime =
          this.OuverturePieton_temporaryOnTimeMS != undefined
            ? this.OuverturePieton_temporaryOnTimeMS
            : 0;
        var c = this.OuverturePieton_SwitchService.getCharacteristic(
          Characteristic.On
        );
        setTimeout(
          c.setValue.bind(c, true),
          additionalTime + this.OuverturePieton_autoCloseS * 1000
        );
      }
    }
  ...
  getServices: function () {
    ...
    this.GarageDoorOpenerService.getCharacteristic(TimesOpened).on(
      "get",
      function (callback) {
        return callback(null, Math.floor(this.Contact_changesCounter / 2));
      }.bind(this)
    );
    ...
    this.OuverturePieton_SwitchService = new Service.Switch(
      "Ouverture Piéton",
      "button2"
    );
